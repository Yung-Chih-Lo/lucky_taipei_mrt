...
捷運與台鐵的 ResultDisplay 元件 SHALL 在 station id 已知後 fetch relay API。若 `excerpt` 不為 null，SHALL 在結果頁顯示「上一個旅人說：{excerpt}」的樣式區塊。若 `excerpt` 為 null 或 fetch 失敗，該區塊 SHALL 靜默不顯示，不影響既有 UI 佈局。
